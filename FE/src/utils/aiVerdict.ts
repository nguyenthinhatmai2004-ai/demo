...
import { calculateCANSLIMScore, calculateSEPAScore, calculateTotalAlphaScore, getCatalystBadges } from './stockScoring';
...
export const generateStockThesis = (stock: StrategicStock): string[] => {
  const catalysts = getCatalystBadges(stock).slice(0, 3).join('; ');
  return [
    `Luận điểm: ${stock.company} phù hợp với bối cảnh ${stock.cycleFit.toLowerCase()}.`,
    `Catalyst theo mã: ${catalysts || 'chưa có catalyst riêng đủ mạnh, cần đợi KQKD và dòng tiền xác nhận'}.`,
